            ...
            <form name="form_master_module" id="form_master_module" method="post">
             <?php echo $message; ?>
              <div class="form-group">
                ...
                <input type="text" class="form-control" placeholder="Enter category name" name="field_val" id="field_val" value="<?php echo set_value('field_val'); ?>">
              </div>
                                 ...
                                 <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $cat_name; ?></td>
                                       	<td>
											<?php if($status==1) { ?>
												<span class="status" data-id="<?php echo "status-".$cat_id; ?>"><a href="javascript:void(0)" title="Status is active. Click here to make it inactive."><span class="label-custom label label-success">Active</span></a></span>
											<?php } else { ?>
												<span class="status" data-id="<?php echo "status-".$cat_id; ?>"><a href="javascript:void(0)" title="Status is inactive. Click here to make it active."><span class="label-custom label label-danger">Inactive</span></a></span>
                                            <?php } ?>
										</td>
                                       	<td>
                                          	<a href="javascript:void(0)" data-id="<?php echo $cat_id; ?>" class="btn btn-success btn-sm edit" title="Edit"><i class="fa fa-pencil"></i></a>
											<a onClick="return confirm('Are you sure to delete this category?')" href="<?php echo base_url().'admin/category/delete/'.$cat_id; ?>" class="btn btn-danger btn-sm" title="Delete"><i class="fa fa-trash-o"></i> </a>
                                       	</td>
                                    </tr>
                                    <?php
										}
...
$(document).on('click', '.edit', function(){
	...
	$.ajax({
		url: "<?php echo base_url(); ?>admin/category/edit_pop/"+val,
		type: 'post',
		cache: false,
		processData: false,
...
$(document).on('click', '.status', function(){
	if(confirm('Are you sure to change the status?'))
	{
		...
		$.ajax({
			url: "<?php echo base_url(); ?>admin/category/changestatus/"+id,
			type: 'post',
			cache: false,
			processData: false,
